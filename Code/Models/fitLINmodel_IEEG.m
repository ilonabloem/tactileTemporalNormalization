function out = fitLINmodel_IEEG(freeParams, fixedParams)
...
switch mode
    ...
    case 'initialize'
        ...
        switch fixedParams{6}
            case 'singleGamma'
                ...
                out.init     = [0.05, 0.06, 3]; % model start params from x0
                out.lb       = [0.001, 0, 0.01]; % lower bound
                out.ub       = [0.5,  0.1, 50]; % upper bound
                out.labels   = {'tau1','shift','scale'};

                if strcmp(fixedParams{5}, 'bads')
                    out.plb      = [0.01, 0.01, 0.5]; % plausible lower bound
                    out.pub      = [0.3, 0.08, 20]; % plausible upper bound                
                end

            case 'doubleGamma'
                out.opts     = optimset('display','off');
                out.init     = [0.05, 0.2,  0.06, 3]; % model start params from x0
                out.lb       = [0.001, 0,  0, 0.01]; % lower bound
                out.ub       = [0.5, 1,  0.1, 50]; % upper bound
                out.labels   = {'tau1','weight','shift','gain'};
                if strcmp(fixedParams{5}, 'bads')
                    out.plb      = [0.01, 0.1, 0.01, 0.5]; % plausible lower bound
                    out.pub      = [0.3, 0.5,  0.08, 20]; % plausible upper bound                
                end
        end
    ...
    case{'optimize', 'prediction'}
        ...
        y_est        = LINmodel_IEEG(freeParams, fixedP);
        ...
        elseif strcmp(mode, 'prediction')
            ...
            switch fixedParams{6}
                case 'singleGamma'
                    out.labels      = {'tau1','shift','scale'};
                case 'doubleGamma'
                    out.labels      = {'tau1','weight','shift','scale'};
            end
