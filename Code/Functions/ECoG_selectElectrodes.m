...
switch method
    ...
    case 'thresh'
        ...
        if ~isfield(opts, 'elec_max_thresh') || ~isfield(opts, 'elec_mean_thresh')
            ...
            % if empty set threshold based on distribution of max responses
            % across electrodes
        end
        ...
        max_resp    = max(mean_resp(stim_on_idx,:),[],1);
        avg_resp    = mean(mean_resp(stim_on_idx,:),1);

        if isempty(opts.elec_max_thresh)
            opts.elec_max_thresh = median(max_resp) + std(max_resp);
        end

        chan_idx(:,1) = max_resp > opts.elec_max_thresh;
        chan_idx(:,2) = avg_resp > opts.elec_mean_thresh;
